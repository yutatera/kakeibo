...
for my in ['month', 'year']:
    for slct in ["asset", "inout", "in", "out1"]:
        out2Num = "None"
        txt = slct + "," + out2Num
        exec('outIndex["/getTable_index/{}/{}"] = json.loads(getTable_index("{}","{}"))'.format(my, txt, my, txt))
        txt = slct + "," + str(out2Num)
        exec('outIndex["/getGraph_index/{}/{}"] = json.loads(getGraph_index("{}","{}"))'.format(my, txt, my, txt))
    slct = "out2"
